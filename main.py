...
def main() -> None:
    ...
    for k in [5, 10, 20, 50, 100, 200, 500]:
        ...
        # fit the model
        svm.fit(
            X_train_new, y_train,C=C,
            learning_rate=learning_rate,
            num_iters=num_iters,
        )

        # evaluate the model
        accuracy = svm.accuracy_score(X_test_new, y_test)
        precision = svm.precision_score(X_test_new, y_test)
        recall = svm.recall_score(X_test_new, y_test)
        f1_score = svm.f1_score(X_test_new, y_test)
